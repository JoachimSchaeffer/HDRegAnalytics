...
class Featlin(): 
    ...
    def analyze_all_features(
        ...
        labels = [f"{chr(97+i)}.{j})" for i in range(len(self.nullspace_dict.keys())) for j in range(1, 5)]
        ...
        if not fig_props['multiple_fig']: 
            fig = plt.figure(constrained_layout=True, figsize=(26,4.72*len(self.nullspace_dict.keys())))
            subfigs = fig.subfigures(nrows=len(self.nullspace_dict.keys()), ncols=1)

        for i, key in enumerate(self.nullspace_dict.keys()): 
            self.analyze_feature(key, opt_cv=opt_cv, opt_dist=opt_dist, plot_cv=0, con_thres=con_thres, opt_gamma_method=opt_gamma_method, std=std, verbose=verbose)
    ...
    def analyze_feature(
        self, feat_key, std=False, plot_cv=0, con_thres=0.01, opt_gamma_method='Xv',
        ...
        for i, model in enumerate(models):
            ...
            nulls_ = nulls_.nullspace_correction(
                ...
                plot_results=False, save_plot=0, con_thres=con_thres, opt_gamma_method=opt_gamma_method, multi_gammas=False, verbose=verbose)

            self.nullspace_dict[feat_key][model_names[i]]['nulls'] = nulls_
            label = format_label(nulls_.max_gamma, method=nulls_.opt_gamma_method, con_val=nulls_.con_val)
            self.nullspace_dict[feat_key][model_names[i]]['nulls_label'] = label

        return self

    def linearization_plot(self, feat_key, axs=None, fig=None, std=False, label_dict: dict=None, gray_line_params=[1, 100, 5]): 
        ...
        if axs is None:
            fig, axs = plt.subplots(1, 4, gridspec_kw={'width_ratios': [4, 2.5, 2.5, 2.5]}, figsize=(23,4.72))
        ...
        axs[0].plot(
            ...
            label=r'$\beta_{T1}$: Feature coefficients', color='k')
        ...
        for j, model_name in enumerate(model_names):
            ...
            if 'PLS' in model_name:
                marker = 's'
                color = '#332288'
                label = model_names[j].split()[0] + ', ' + model_names[j].split()[1] + ' ' + model_names[j].split()[2] + '.'
            if 'RR' in model_name:
                marker = 'o'
                color = '#117733'
                ...
                marker = 's'
            axs[0].plot(
                self.data.x, reg.coef_.reshape(-1), label=r"$\beta$:" + ' ' + label, lw=2, 
                color=color, 
                marker=marker, markevery=(5*(j+1), 65), markersize=5)
                # , NRMSE: {nrmse_reg:.2f}%
            ...
            axs[0].plot(
                self.data.x, reg.coef_.reshape(-1)+nulls_.nullsp['v_'][-1,:], label=r'$\beta + \mathbf{v}:$' + ' ' + label.split()[-2] + ' ' + label.split()[-1], lw=2, 
                color='#AA4499', 
        ...
        if std:
            ...
            axs[0].fill_between(x.reshape(-1), nulls_.nullsp['w_alpha_std'], y2=y2, color='darkgrey', zorder=-1, alpha=0.35)#, label=label)
        else:
            y2 = nulls_.nullsp['w_alpha']+nulls_.nullsp['v_'][-1,:]
            axs[0].fill_between(x.reshape(-1), nulls_.nullsp['w_alpha'], y2=y2, color='darkgrey', zorder=-1, alpha=0.35)#, label=label)
        ...
        plot_linearized_nonlinear_comp(
            ...
            title='', xlabel='Linearized Feature', ylabel='Feature', ax=axs[2])

        # Right: Pearson correlation coefficient
        plot_pearson_corr_coef_comp(
            feat_nonlin,  self.data.y_, self.cmap, title='Person Correlation', xlabel='Feature', ylabel='y', ax=axs[3])
