...
def generate_wide_datamatrix(fun, range_x, range_m, columns, rows, snr_x=-1):
    ...
    ---------
    fun : callable 
        Function that is defined for all values inside range. Representing a measurement that was taken from any process. 
    range_x : ndarray
        1D array with two elements, the first being strictly smaller than the second, defining the meaning of the columsn
    range_m : ndarray 
        1D array with two elements, range of factor
        each row of x is equal to m*fun(x), where m is sampled from a uniform distribution
    datapoints : int
        number of linearly spaced datapoints for evaluating fun, equal to number of columns of X
    rows : int 
        number of rows of X, i.e. 
    snr_x : int, default=-1
        Signal to noise ratio of AWGN to be added on the signal, if -1, then this functions adds no noise to the signal
...
def generate_target_values(X, targetfun, percentage_range_x_to_t=[0,1]):
    ...
    ---------
    X : ndarray
        2D array \in R^mxn with n>>m (wide data matrix, used as input for the targetfunction y
    targetfun : callable
